...
<template>
  <PageLayout navTitle="收件人信息"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TnRecipientData';
defineOptions({
  name: 'TnRecipientList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/receiver/tnRecipient/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/receiver/tnRecipient/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
