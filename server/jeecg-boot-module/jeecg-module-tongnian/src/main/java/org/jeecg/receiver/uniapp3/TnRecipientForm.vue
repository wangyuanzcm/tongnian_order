        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('收件人姓名')"
                  name='name'
                  prop='name'
                  placeholder="请选择收件人姓名"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('收件人手机号')"
                  name='phone'
                  prop='phone'
                  placeholder="请选择收件人手机号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-pca
                :label="get4Label('所在地区')"
                 labelWidth="100px"
                 name='address'
                 prop='address'
                 clearable
                v-model:value="myFormData['address']"
              ></online-pca>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['addressDetail']"
                  :label="get4Label('地址详情')"
                  name='addressDetail'
                  prop='addressDetail'
                  placeholder="请选择地址详情"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['nickName']"
                  :label="get4Label('收件人昵称')"
                  name='nickName'
                  prop='nickName'
                  placeholder="请选择收件人昵称"
                  :rules="[
                  ]"
                  clearable
              />
...
const dataId = ref('')
const backRouteName = ref('TnRecipientList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/receiver/tnRecipient/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/receiver/tnRecipient/edit':'/receiver/tnRecipient/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
